<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:78:"/Users/dylan/PHP/Project/shop/public/../application/admin/view/index/menu.html";i:1547890275;}*/ ?>
...
.explode {
  background:url(/static/admin/Images/menu_minus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.collapse {
  background:url(/static/admin/Images/menu_plus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.menu-item {
  background:url(/static/admin/Images/menu_arrow.gif) no-repeat 0px 3px;
  font-weight:normal;
}
#help-title {
  font-size:14px;
    ...
    <p>
        <span style="float:right; padding:3px 5px;" >
            <a href="javascript:toggleCollapse();">
                <img id="toggleImg" src="/static/admin/Images/menu_minus.gif" width="9" height="9" border="0" alt="闭合" />
            </a>
        </span>
            ...
            <?php echo $vo['rule_name']; ?>
                <ul>
                  <?php if(is_array($menus) || $menus instanceof \think\Collection || $menus instanceof \think\Paginator): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;if($v['parent_id'] == $vo['id']): ?>
                    <li class="menu-item"><a href="<?php echo url('admin/'.$v['controller_name'].'/'.$v['action_name']); ?>" target="main-frame"><?php echo $v['rule_name']; ?></a></li>
